...
public interface BookObjRepository extends CrudRepository<BookObj, Integer> {
	...
	@Query(value="SELECT b.* FROM book_obj b LEFT OUTER JOIN issue i ON b.id = i.book_obj_id WHERE b.book_isbn = ?1 AND (i.status IS NULL OR i.status != 2) LIMIT 1", nativeQuery=true)
